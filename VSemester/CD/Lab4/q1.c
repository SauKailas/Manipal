...
const char *keywords[] = {"auto","double","int","struct","break","else",
"long","switch","case","enum", "register","typedef","char","extern","return",
"union","continue","for","signed","void","do","if","static","while","default",
"goto","sizeof","volatile","const","float","short","unsigned","printf","scanf",
"true","false"};
...
int main()
{
    ...
    while((token = getNextToken(fptr)).row != -1)
    {
        ...
        else if(strcmp(token.type, "IDENTIFIER") == 0)
        {
            ...
            else if((strcmp(token.type, "LS")) == 0)
            {
                if(findTable(st[i], w, j) == 0)
                {
                    token = getNextToken(fptr);
        ...
        else if(strcmp(token.type, "LC") == 0)
            flag++;
        else if(strcmp(token.type, "RC") == 0)
        {
            flag--;
            if(flag == 0)
            {
