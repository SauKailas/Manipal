...
const char *keywords[] = {"int", "char", "if", "else", "do", "while", "for", "switch", "case",
							"break", "return", "unsigned", "const"};
...
void insertIntoTable(struct token *tkn, int no_args, char *type)
{
...
{
	printf("S.No\tLexemeName\tDataType\tReturnType\tNoArgs\tTokenType\n");
	for(int i=0; i<n; i++)
	{
...
struct token getNextToken(FILE *f1)
{
	...
	while(gotToken == 0 && (c = fgetc(f1)) != EOF)
	{
		...
		else if(c == '<' || c == '>' || c == '!')
		{
			...
			{
				if(d == '!')
					strcpy(tkn.lexeme, "LOGICAL OPERATOR");
