...
__global__ void matrix_mult_rowwise(int *A, int *B, int *C, int wa, int wb)
{
	int rowIdA = blockIdx.x * blockDim.x + threadIdx.x;
	int sum;
...
int main()
{
	...
	dim3 dimGrid(1, 1, 1);
	dim3 dimBlock(ha, 1, 1);
	matrix_mult_rowwise<<<dimGrid, dimBlock>>>(d_A, d_B, d_C, wa, wb);
